...
-   [twitter-and-us-lawmakers](#twitter-and-us-lawmakers)
    ...
    -   [II Thoughts corpus
        compilation](#ii-thoughts-corpus-compilation)
    -   [III A shiny app for Twitter corpus
        search](#iii-a-shiny-app-for-twitter-corpus-search)
    -   [IV Summary](#iv-summary)

I Twitter handles for US lawmakers
----------------------------------

Building a Twitter handle list for US lawmakers. With important metadata
for cross-walking to other political data sets.
...
```
    ...
    ## [1] "/tmp/RtmpO4t0Wx/HS115_members.csv"
    ## [1] "/tmp/RtmpO4t0Wx/HS116_members.csv"
...
II Thoughts corpus compilation
------------------------------
...
Instead, a patchwork solution:

-   Older tweets can be obtained from, eg, [the George Washington
    University (GWU) Library](https://tweetsets.library.gwu.edu/), which
    makes available Tweet IDs for tweets generated by US lawmakers by
    congress & chamber. Tweets can then be “hydrated” using a nifty
    application developed and made publicly available from
    [DocNow](https://github.com/DocNow/hydrator).

-   More recent - and real-time - tweets via the R package `rtweet`.

------------------------------------------------------------------------

III A shiny app for Twitter corpus search
-----------------------------------------
